...
[Step 3-A] Firestore 조회 (Cloud Function이 트리거 시점에 저장한 메타)
    ├─ 데이터 있음 → [Step 4] 포맷팅 (Firestore — title/subtitle/version)
...
### Step 3-A: Firestore 조회

BigQuery `stats_rows`가 비어있으면 Firestore에서 Cloud Function이 저장한 메타데이터를 조회한다.

```python
from google.cloud import firestore
db = firestore.Client(project="market-kurly")
doc = db.collection("crashlytics_issues").document(ISSUE_ID).get()
if doc.exists:
    data = doc.to_dict()
    # data keys: issue_id, title, subtitle, app_version, url, event_type,
    #            created_at, jira_key, has_bigquery_data,
    #            stack_trace (있으면), device_info (있으면)
```

**Firestore 데이터가 있는 경우:**
...
**Firestore 데이터가 없는 경우:** Step 3-B (Firebase Console fallback) 진행.

### Step 3-B: Firebase Console Fallback

BigQuery, Firestore 모두 데이터가 없으면 Firebase Console에서 직접 수집한다.
